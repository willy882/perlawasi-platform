    ...
    const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (editingProduct?.id) {
                const { error } = await supabase.from('licoreria').update(productData).eq('id', editingProduct.id)
                if (error) throw error
                toast.success('Licor actualizado 🥂')
            } else {
                const { error } = await supabase.from('licoreria').insert([productData])
                if (error) throw error
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h2 className="text-3xl font-display font-black text-gray-900 leading-none">Licorería Amazónica</h2>
                    <p className="text-gray-500 mt-2 text-sm font-medium italic">Destilados y macerados regionales.</p>
                </div>
                <button onClick={() => handleOpenModal()} className="flex items-center gap-2 px-6 py-3.5 bg-indigo-900 text-white rounded-2xl font-bold text-sm shadow-xl shadow-indigo-900/20 hover:bg-black transition-all">
                    <FiPlus /> Nuevo Licor
                </button>
            </div>
                    ...
                    <div className="bg-white w-full max-w-xl rounded-[3rem] p-10 animate-slide-up max-h-[90vh] overflow-y-auto">
                        <div className="flex justify-between items-center mb-8">
                            <h3 className="text-2xl font-black text-gray-900">
                                {editingProduct?.id ? 'Editar Licor' : 'Nuevo Licor Regional'}
                            </h3>
